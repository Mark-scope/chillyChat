...
import hand from "../assets/8.png";
// import Button from '../components/Button';

const Signup = () => {
  const [currentImage, setCurrentImage] = useState(Loginicon);

  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentImage((prevImage) => (prevImage === Loginicon ? hand : Loginicon));
    }, 3000);
  ...
  return (
    <div className='flex'>
      <div className='w-1/2 text-white flex flex-col justify-center items-center bg-custom-gradient h-screen'>
        <img className='pb-12 pr-16 mt-10' src={currentImage} alt="" />
      ...
      <div className='w-1/2 flex flex-col justify-center items-center space-y-10'>
        ...
        <div>
          ...
          <p className='text-center mt-4'>Already have an account? <a href="/Login" className='text-violet cursor-pointer'>Login now</a></p>
        </div>
      </div>
    </div>
  );
